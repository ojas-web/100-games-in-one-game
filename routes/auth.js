...
function publicUser(user) { const { passwordHash, sessionToken, ...safe } = user; return safe; }
...
router.post('/signup', async (req, res) => {
  ...
  try {
    await updateJson('users.json', users => {
      ...
      user = { id: randomUUID(), playFabId: randomUUID().replace(/-/g, ''), username, passwordHash: hashPassword(req.body.password), sessionToken: randomUUID(), joinDate: new Date().toISOString(), lastLogin: new Date().toISOString(), totalScore: 0, gamesPlayed: 0, highestScore: 0, favoriteGame: '', favorites: [], recentGames: [], xp: 0, level: 1, badges: [], achievements: ['First Login'], isAdmin: users.length === 0 };
      return [...users, user];
    });
...
router.get('/profile/:username', (req, res) => {
  ...
  res.json({ user: { ...publicUser(user), achievements: achievementsFor(user), ranking } });
});
